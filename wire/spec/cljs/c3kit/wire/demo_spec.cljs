		...
		(context "content"
				...
				(for [truck @sut/monster-trucks]
						(it (str "monster trucks shows: " (:name truck))
								(should-select (str "#-truck-" (string/replace (:name truck) #"\s" "-")))))
				)
				...
				(context "golf"
						...
						(it "drags ball to hole then out of hole"
								...
								(let [golf-locations [(:ball-location @sut/golf-state) (:hole-location @sut/golf-state)]]
										...
										(map #(should-be-nil (% @sut/golf-state)) [:dragging :hove :drop-hole])))
		...
		(context "lists"
				(context "Rainbow - single list"
						...
						(it "colors are draggable"
								(let [draggables [:node :draggable-mousedown :draggable-touchstart :droppable-mouseenter :droppable-mouseleave :droppable-touchend]]
						...
						(it "colors are droppable"
								(let [droppables [:node :draggable-mousedown :draggable-touchstart :droppable-mouseenter :droppable-mouseleave :droppable-touchend]]
										(should= [:red :orange :yellow :green :blue :indigo :violet :after] (keys (get-in @sut/rainbow-dnd [:groups :color :members])))
						...
						(it "drags red to indigo then out of indigo"
								...
								(swap! sut/rainbow-dnd assoc :target-key :indigo)
										...
										(sut/color-drag-end @sut/rainbow-dnd)
										(map #(should-be-nil (% @sut/rainbow-state)) [:dragging :hove :drop-color]))
				...
				(context "Monster Jam - multiple lists"
						...
						(it "gets item order"
								(let [first-element-id (sut/get-first-element-id sut/teams-state :trucks)
														first-element    (sut/get-first-element sut/teams-state @sut/monster-trucks :trucks)
						...
						(context "update truck order"
								(it "moves to nil drop point"
										...
										(should= :megalodon (get-in @sut/teams-state [:trucks :first-item]))
										(let [items (sut/get-items-order @sut/monster-trucks :trucks (sut/get-first-element sut/teams-state @sut/monster-trucks :trucks))]
								...
								(context "moves within same list"
										(it "moves one before three (move first element down one)"
												(sut/truck-drag-started {:source-key :megalodon})
												(sut/update-order {:source-key :megalodon :target-key :dragon} sut/teams-state sut/monster-trucks)
												(should= :son-uva-digger (get-in @sut/teams-state [:trucks :first-item]))
												(let [items (sut/get-items-order @sut/monster-trucks :trucks (sut/get-first-element sut/teams-state @sut/monster-trucks :trucks))]
										...
										(it "moves two before four"
												(sut/truck-drag-started {:source-key :son-uva-digger})
												(sut/update-order {:source-key :son-uva-digger :target-key :grave-digger} sut/teams-state sut/monster-trucks)
												(should= :megalodon (get-in @sut/teams-state [:trucks :first-item]))
												(let [items (sut/get-items-order @sut/monster-trucks :trucks (sut/get-first-element sut/teams-state @sut/monster-trucks :trucks))]
										...
										(it "moves two to last"
												(sut/truck-drag-started {:source-key :son-uva-digger})
												(sut/update-order {:source-key :son-uva-digger :target-key :after-trucks} sut/teams-state sut/monster-trucks)
												(should= :megalodon (get-in @sut/teams-state [:trucks :first-item]))
												(let [items (sut/get-items-order @sut/monster-trucks :trucks (sut/get-first-element sut/teams-state @sut/monster-trucks :trucks))]
										...
										(it "moves one to last"
												(sut/truck-drag-started {:source-key :megalodon})
												(sut/update-order {:source-key :megalodon :target-key :after-trucks} sut/teams-state sut/monster-trucks)
												(should= :son-uva-digger (get-in @sut/teams-state [:trucks :first-item]))
												(let [items (sut/get-items-order @sut/monster-trucks :trucks (sut/get-first-element sut/teams-state @sut/monster-trucks :trucks))]
										...
										(it "moves three to first"
												...
												(sut/update-order {:source-key :dragon :target-key :megalodon} sut/teams-state sut/monster-trucks)
										...
										(it "moves four to first"
												...
												(sut/update-order {:source-key :grave-digger :target-key :megalodon} sut/teams-state sut/monster-trucks)
										...
										(it "moves three before two"
												(sut/truck-drag-started {:source-key :dragon})
												(sut/update-order {:source-key :dragon :target-key :son-uva-digger} sut/teams-state sut/monster-trucks)
												(should= :megalodon (get-in @sut/teams-state [:trucks :first-item]))
												(let [items (sut/get-items-order @sut/monster-trucks :trucks (sut/get-first-element sut/teams-state @sut/monster-trucks :trucks))]
														(should= ["Megalodon" "Dragon" "Son-Uva-Digger" "Grave Digger"] (map :name items))))
										)
								...
								(context "moves to a different list"
										(it "moves two as first element of second list"
												(sut/truck-drag-started {:source-key :son-uva-digger})
												(sut/update-order {:source-key :son-uva-digger :target-key :team} sut/teams-state sut/monster-trucks)
												(should= :megalodon (get-in @sut/teams-state [:trucks :first-item]))
												(should= :son-uva-digger (get-in @sut/teams-state [:team :first-item]))
												(let [truck-items    (sut/get-items-order @sut/monster-trucks :trucks (sut/get-first-element sut/teams-state @sut/monster-trucks :trucks))
																		team-items     (sut/get-items-order @sut/monster-trucks :team (sut/get-first-element sut/teams-state @sut/monster-trucks :team))
										...
										(it "moves two before existing first element in second list"
												...
												(swap! sut/teams-state assoc-in [:team :first-item] :earth-shaker)
												(sut/truck-drag-started {:source-key :son-uva-digger})
												(sut/update-order {:source-key :son-uva-digger :target-key :earth-shaker} sut/teams-state sut/monster-trucks)
												(should= :megalodon (get-in @sut/teams-state [:trucks :first-item]))
												(should= :son-uva-digger (get-in @sut/teams-state [:team :first-item]))
												(let [truck-items    (sut/get-items-order @sut/monster-trucks :trucks (sut/get-first-element sut/teams-state @sut/monster-trucks :trucks))
																		team-items     (sut/get-items-order @sut/monster-trucks :team (sut/get-first-element sut/teams-state @sut/monster-trucks :team))
										...
										(it "moves two between two existing elements in second list"
												...
												(swap! sut/teams-state assoc-in [:team :first-item] :earth-shaker)
												(sut/truck-drag-started {:source-key :son-uva-digger})
												(sut/update-order {:source-key :son-uva-digger :target-key :monster-mutt-rottweiler} sut/teams-state sut/monster-trucks)
												(should= :megalodon (get-in @sut/teams-state [:trucks :first-item]))
												(should= :earth-shaker (get-in @sut/teams-state [:team :first-item]))
												(let [truck-items    (sut/get-items-order @sut/monster-trucks :trucks (sut/get-first-element sut/teams-state @sut/monster-trucks :trucks))
																		team-items     (sut/get-items-order @sut/monster-trucks :team (sut/get-first-element sut/teams-state @sut/monster-trucks :team))
										...
										(it "moves first element of list 1 to first element of list 2"
												(sut/truck-drag-started {:source-key :megalodon})
												(sut/update-order {:source-key :megalodon :target-key :team} sut/teams-state sut/monster-trucks)
												(should= :son-uva-digger (get-in @sut/teams-state [:trucks :first-item]))
