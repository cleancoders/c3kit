...
(defn test-content []
  [:div
   [:div
    [:h1 "treats"]
    [:ol#-treats
     [:li#-bone.treat {:ref (sut/register dnd :treat "bone") :style {:x 0 :y 0 :width 100 :height 50}}]
     [:li#-catnip {:ref (sut/register dnd :treat "catnip") :style {:x 0 :y 50 :width 100 :height 50}}]
     [:li#-brusly.pet {:ref (sut/register dnd :pet "brusly") :style {:x 0 :y 150 :width 100 :height 50}}]
     [:li#-cheddar.pet {:ref (sut/register dnd :pet "cheddar") :x 0 :y 200 :style {:x 0 :y 200 :width 100 :height 50}}]
     ]]])
  ...
  (context "valid dnd"
    ...
    (context "dnd behaviors"
      ...
      (context "dnd actions"
        ...
        (context "mouse actions"
          (context "draggable-mouse-down"
            (it "right mouse-down"
              (println "@dnd: " @dnd)
              (println "@draggers: " @draggers)
              (println "@bone: " @bone)
          ...
          (context "dragging"
            ...
            (it "not start-drag with false dispatch event"
              (with-redefs [sut/dispatch-event (stub :dispatch {:return false})]
                (sut/start-drag dnd :treat "bone" @bone-node [sut/handle-mouse-drag sut/end-mouse-drag] (clj->js {}))
            ...
            (it "start drag"
              (with-redefs [wjs/node-bounds (stub :bounds {:return [10 10]})]
                (sut/start-drag dnd :treat "bone" @bone-node [sut/handle-mouse-drag sut/end-mouse-drag] (clj->js {:target @bone-node :clientX 0 :clientY 0 :scrollX 10 :scrollY 10}))
            ...
            (context "drag node"
              (it "is cloned when no hiccup-fn"
                (with-redefs [wjs/node-bounds (stub :bounds {:return [10 10]})
                              sut/fake-hiccup->dom (stub :fake-hiccup-dom)]
                  (sut/start-drag dnd :treat "bone" @bone-node [sut/handle-mouse-drag sut/end-mouse-drag] (clj->js {:target @bone-node :clientX 0 :clientY 0 :scrollX 10 :scrollY 10}))
              ...
              (it "is created when hiccup-fn - when node has no classes"
                (with-redefs [wjs/node-bounds (stub :bounds {:return [10 10]})]
                  (sut/drag-fake-hiccup-fn dnd :treat fake-hiccup)
                  (should-not-be-nil (get-in @dnd [:groups :treat :hiccup]))
                  (sut/start-drag dnd :treat "catnip" @catnip-node [sut/handle-mouse-drag sut/end-mouse-drag] (clj->js {:target @catnip-node :clientX 0 :clientY 0 :scrollX 10 :scrollY 10}))
                  (let [node (get-in @dnd [:active-drag :drag-node])]
                    (should= "_dragndrop-drag-node_" (helper/id node))
                    (should-contain "_dragndrop-drag-node_" (map #(helper/id %) (wjs/node-children (wjs/doc-body (wjs/document)))))
                    (should-contain "drag-bone" (clojure.string/split (wjs/node-classes node) #" "))
      ...
      #_(context "mobile touch actions"
          (context "touchstart"
            (it "more than single touch"
              (let [cnt-add-listeners (count (stub/invocations-of :add-listener))
                    touchstart        (:draggable-touchstart @dragger)]
                (touchstart (clj->js [{:clientX 0 :clientY 0} {:clientX 10 :clientY 10}]))
                (should= 0 (- (count (stub/invocations-of :add-listener)) cnt-add-listeners))
                (should-not-contain :maybe-drag @dnd)))
            ...
            (it "single touch"
              (let [before-listeners (get-listeners :add-listener)
                    touchstart       (:draggable-touchstart @dragger)
                    _                (touchstart (clj->js [{:clientX 0 :clientY 0}]))
                    after-listeners  (get-listeners :add-listener)]
                (should= 1 (- (count after-listeners) (count before-listeners)))
