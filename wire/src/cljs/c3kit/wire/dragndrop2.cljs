...
(def dnd-structure
  {:groups      {:group-key-1 {:members    {:member-key-1 {:node                 "dom node"
                                                           ...
                                                           :touchable-touchstart "touchstart listener to start drag"
...
(defn touch-move [dnd group member node js-event]
  ...
  (when-let [touches (.-touches js-event)]
    (when (>= (.-length touches) 5)
      (let [[start-x start-y] (-> @dnd :maybe-drag :start-position)
        last-touch          (last touches)
        [x y] [(.-clientX last-touch) (.-clientY last-touch)]
        ;[x y] (wjs/e-coordinates js-event)
    ...
    (println "[x y] distance above-threshold?: " [x y] distance above-threshold?)
    (when above-threshold?
      (do (start-drag dnd group member node js-event)
        (end-mouse-maybe-drag dnd nil))
      (wjs/nod js-event))))))

(defn touchable-touchstart [dnd group member node js-event]
  (println "touch start!")
  (println "(.-touches js-event): " (.-touches js-event))
  (println "(count (.-touches js-event)): " (.-length (.-touches js-event)))
  (when-let [touches (.-touches js-event)]
    (when (= 1 (.-length touches))
      (let [listener       (partial touch-move dnd group member node)
            ...
            doc            (wjs/document node)
            touch        (first touches)
            start-position [(.-clientX touch) (.-clientY touch)]
            ]
...
(defn maybe-make-draggable! [{:keys [node] :as data} dnd group member]
  (if (seq (get-in @dnd [:groups group :targets]))
    (let [mousedown  (partial draggable-mouse-down dnd group member node)
          touchstart (partial touchable-touchstart dnd group member node)]
      (wjs/add-listener node "mousedown" mousedown)
      (wjs/add-listener node "touchstart" touchstart)
      (assoc data :draggable-mousedown mousedown :touchable-touchstart touchstart))
    data))
...
(defn register [dnd group member]
  (fn [node]
    ...
    (if node
      ...
      (let [{:keys [node draggable-mousedown touchable-touchstart droppable-mouseenter droppable-mouseleave droppable-touchend] :as drag}
            (get-in @dnd [:groups group :members member])]
        (when draggable-mousedown (wjs/remove-listener node "mousedown" draggable-mousedown))
        (when touchable-touchstart (wjs/remove-listener node "touchstart" touchable-touchstart))
